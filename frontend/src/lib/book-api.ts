...
// Shared mock data storage for persistence across CRUD operations
let sharedMockBooks: Book[] = [];
...
export const bookApi = {
  ...
  async getBooks(filter?: BookFilter): Promise<Book[]> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return data.books || data; // Handle both response formats
      }
    } catch (error) {
  ...
  async createBook(bookData: BookFormData): Promise<Book> {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('Book created via backend:', data);
        return data;
      }
    } catch (error) {
  ...
  async updateBook(id: string, bookData: Partial<BookFormData>): Promise<Book> {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('Book updated via backend:', data);
        return data;
      }
    } catch (error) {
    ...
    const bookIndex = sharedMockBooks.findIndex((b: Book) => b._id === id);
    if (bookIndex === -1) throw new Error('Book not found');
    
    const updatedBook = { 
      ...sharedMockBooks[bookIndex], 
      ...bookData, 
      publishedYear: bookData.publishedYear ? parseInt(bookData.publishedYear) : sharedMockBooks[bookIndex].publishedYear,
      updatedAt: new Date() 
    };
    
    // Update in shared storage
    sharedMockBooks[bookIndex] = updatedBook;
  ...
  async deleteBook(id: string): Promise<void> {
    ...
    try {
      ...
      if (response.ok) {
        console.log('Book deleted via backend');
        return;
      }
    } catch (error) {
    ...
    sharedMockBooks = sharedMockBooks.filter((b: Book) => b._id !== id);
  ...
  generateNextBookId(category: string, books: Book[]): string {
    const categoryBooks = books.filter(book => book.bookId.startsWith(category));
    const maxId = categoryBooks.reduce((max, book) => {
      const idNumber = parseInt(book.bookId.substring(2));
      return idNumber > max ? idNumber : max;
    }, 0);
    
    const nextId = maxId + 1;
    return `${category}${nextId.toString().padStart(3, '0')}`;
  },

  // Mock data for testing when backend is not available
  getMockBooks(filter?: BookFilter): Book[] {
    // Initialize shared storage if empty
    if (sharedMockBooks.length === 0) {
      sharedMockBooks = [
        {
          _id: '1',
          bookId: 'TB001',
          ...
          author: 'S. Ramanathan',
          publishedYear: 2020,
          description: 'A comprehensive collection of classical Tamil literature including Sangam poetry, Bhakti literature, and modern works.',
          imageUrl: 'https://via.placeholder.com/300x400/FF6B35/FFFFFF?text=Tamil+Classics',
    ...
    let filteredBooks = sharedMockBooks;
